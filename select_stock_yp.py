...
def load_data(data_dir: Path, codes: Iterable[str]) -> Dict[str, pd.DataFrame]:
    ...
    for code in codes:
        ...
        try:
            ...
            logger.error("%s 读取失败：%s", fp.name, e)
            continue
        frames[code] = df
    return frames
...
def save_selection_results(
    ...
    Args:
        ...
        trade_date: 交易日期
        alias: 策略别名
        output_path: 输出文件路径，如果为None则自动生成文件名
    ...
    # 创建结果数据
    result_data = {
        'date': [trade_date.date()] * len(picks_with_suffix),
        'strategy': [alias] * len(picks_with_suffix),
    ...
    if output_path:
        ...
        output_file = Path(f"stock_selection_results_{trade_date.strftime('%Y%m%d')}.csv")
...
def main():
    ...
    for cfg in selector_cfgs:
        ...
        logger.info(" %s %s 选出%s", trade_date.date(), alias, ", ".join(picks) if picks else "无符合条件股票")
        ...
        if picks:
            if args.excel:
                ...
                # 使用CSV格式保存
                save_selection_results(picks, trade_date, alias, args.output)
