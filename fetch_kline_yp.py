...
def _get_mktcap_tushare(trade_date) -> pd.DataFrame:
    # pro = ts.pro_api()

    for attempt in range(1, 4):
        try:
            df = pro.daily_basic(ts_code='', trade_date=trade_date, fields="ts_code,total_mv")
            print(df)
            break
        except Exception as e:
        ...
        raise RuntimeError("Tushare 连续三次拉取市值快照失败！")
    df = df[["ts_code", "total_mv"]].rename(columns={"ts_code": "code", "total_mv": "mktcap"})
    df["mktcap"] = pd.to_numeric(df["mktcap"], errors="coerce")
    df["mktcap"] = df["mktcap"] * 10000 #tushare的市值单位是万元，需要转换为元
    return df
...
def get_constituents(
    ...
    df = mktcap_df if mktcap_df is not None else _get_mktcap_tushare()

    cond = (df["mktcap"] >= min_cap) & (df["mktcap"] <= max_cap)
    if small_player:
        cond &= ~df["code"].str.startswith(("300", "301", "688", "9"))
...
def main():
    ...
    if args.datasource == "tushare":
        ts_token = "da9656165858b6c7e0243929eb60d164f0f8307a35fe5e8f75841467"  # 在这里补充token
    ...
    # ---------- 市值快照 & 股票池 ---------- #
    mktcap_df = _get_mktcap_tushare(end)    
